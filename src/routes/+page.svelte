...
<div
  class="max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl mx-auto px-4 sm:px-6 py-8 flex flex-col space-y-6 font-montserrat bg-[#FFFFFF] border-2 border-gray-600 rounded-xl shadow-sm print:shadow-none print:bg-white print:border-none"
>
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start print:flex">
    <div class="flex flex-col gap-4 w-full sm:w-auto">
      <div>
        {#if appState.company.logo}
          <div class="flex flex-row items-center space-x-4">
            <div class="flex flex-col border-2 border-gray-600 rounded-xl">
              <button
                class="p-2 hover:bg-[#F5F5F5] rounded"
                on:click={() => {
        ...
        {:else}
          <button
            class="p-2 flex flex-row gap-2 rounded-lg border-2 border-gray-600 cursor-pointer hover:bg-[#F5F5F5]"
            on:click={() => document.getElementById('imageInput').click()}
          >
            <ImagePlus />
            <p class="text-sm text-[#0A2E56]">Click to select an image for logo</p>
            <input
              id="imageInput"
              type="file"
              accept=".png,.jpg,.jpeg,.webp"
      ...
      <input
        class="font-bold text-xl border-2 border-gray-600 p-3 rounded-lg focus:outline-none focus:border-[#0A2E56] w-full"
        type="text"
    ...
    <div class="relative flex flex-col items-start sm:items-end gap-2 mt-4 sm:mt-0 w-full sm:w-auto">
      <div class="flex items-center">
        <h2 class="font-bold text-2xl text-[#0A2E56] text-left sm:text-right print:text-left">
          Invoice :
        </h2>
        <input
          type="text"
          ...
          class="border-2 border-gray-600 p-2 rounded-lg focus:outline-none focus:border-[#0A2E56]"
        />
      </div>
      ...
      <select
        ...
        class="border-2 border-gray-600 p-2 rounded-lg focus:outline-none focus:border-[#0A2E56] w-full"
      >
  ...
  <div class="flex flex-col sm:flex-row justify-between gap-4">
    <div class="flex flex-col gap-2 w-full">
      ...
      <textarea
        ...
        class="border-2 border-gray-600 p-2 rounded-lg focus:outline-none focus:border-[#0A2E56] w-full bg-[#F5F5F5]"
        on:input={(e) => autoExpand(e.target)}
        rows="1"
      ></textarea>
      <!-- Other contact details with auto-expanding textareas and adjusted borders -->
      <!-- ... -->
    </div>
    <div class="flex flex-col gap-2 w-full">
  ...
  <form class="flex flex-col sm:flex-row justify-between gap-2">
    <div class="flex items-center">
      <button
        disabled={itemDesc == ''}
        on:click={() => addItem()}
        class="p-2 rounded-full bg-[#00B9FF] hover:bg-[#008FCC] transition-all duration-100 ease-in-out print:hidden"
      >
        <Plus stroke="#FFFFFF" />
      </button>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-2 w-full">
      <textarea
        ...
        class="border-2 border-gray-600 rounded-lg p-3 focus:outline-none focus:border-[#0A2E56] grow w-full resize-none"
        rows="1"
      ...
      <input
        ...
        min="0"
        step="0.01"
        class="border-2 border-gray-600 rounded-lg p-3 focus:outline-none focus:border-[#0A2E56] w-full sm:w-32"
        on:keypress={(e) => e.key == 'Enter' && addItem()}
      />
      <input
        type="number"
        ...
        on:keypress={(e) => e.key == 'Enter' && addItem()}
      />
      <p class="border-2 border-gray-600 p-3 w-full sm:w-32 text-right bg-[#F5F5F5]">
  ...
  <div class="flex flex-col mt-4 p-2 rounded-lg border-2 border-gray-600">
    ...
    <div class="flex flex-row bg-[#F5F5F5] border-2 border-gray-600">
      <p class="font-bold p-3 border-r-2 border-gray-600 grow text-right">Item Name</p>
      <p class="font-bold p-3 border-r-2 border-gray-600 w-32 text-right">Unit Price</p>
    ...
    {#each appState.items as item, index}
      <div class="flex flex-row even:bg-[#00B9FF] border-2 border-gray-600">
        <textarea
          class="p-3 border-r-2 border-gray-600 grow resize-none bg-[#F5F5F5]"
          bind:value={item.desc}
          rows="1"
          on:input={(e) => { autoExpand(e.target); save(); }}
        ></textarea>
        <input
          class="p-3 border-r-2 border-gray-600 w-32 bg-[#F5F5F5] text-right"
          type="number"
          min="0"
          step="0.01"
        ...
        />
        <input
          class="p-3 border-r-2 border-gray-600 w-24 bg-[#F5F5F5] text-right"
          type="number"
          min="0"
          step="1"
          ...
          on:input={() => save()}
        />
        <p class="p-3 w-32 text-right bg-[#F5F5F5]">
          {getCurrencySymbol(appState.currency)}{(item.price * item.quantity).toFixed(2)}
        </p>
      </div>
    {/each}

    <!-- Tax, Discount, and Total Due Sections -->
    <div class="flex flex-row items-center border-t-2 border-gray-600">
      ...
      <div class="relative">
        <input
          class="p-3 w-32 text-right bg-[#F5F5F5] {isDiscountEnabled ? '' : 'opacity-50'}"
          type="number"
    ...
    <div class="flex flex-row items-center border-t-2 border-gray-600">
      ...
      <div class="relative">
        <input
          class="p-3 w-32 text-right bg-[#F5F5F5] {isTaxEnabled ? '' : 'opacity-50'}"
          type="number"
    ...
    <div class="flex flex-row border-t-2 border-gray-600">
      <p class="p-3 grow text-right font-bold bg-[#0A2E56] text-white">Total Due</p>
      <p class="p-3 w-32 text-right bg-[#0A2E56] text-white font-bold">
  ...
  <div class="mt-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <!-- Payment fields with auto-expanding textareas and adjusted borders -->
      <!-- ... -->
    </div>
  </div>
  ...
  <div class="mt-6 flex justify-center print:hidden">
    <button
      ...
      class="px-4 py-3 rounded-lg bg-[#00B9FF] text-white font-semibold flex items-center gap-2 hover:bg-[#008FCC] transition-transform duration-150"
    >

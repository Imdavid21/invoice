...
<div class="max-w-screen-md mx-auto px-6 py-8 flex flex-col space-y-6 font-montserrat bg-[#FAFAFA] border border-[#E2E2E2] rounded-xl shadow-sm print:shadow-none print:bg-white print:border-none">
  ...
  <div class="flex flex-row justify-between items-start print:flex">
    ...
    <div class="relative flex flex-col items-end gap-2 print:items-start print:w-full">
      <h2 class="font-bold text-2xl text-[#1E6F5C] text-right print:text-left">
        INVOICE :
        <input type="text" size="4" placeholder="007" maxlength="4" bind:value={appState.invoice.number} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </h2>
      <button on:click={() => reset()} class="p-2 rounded-lg hover:bg-[#E2E2E2] transition-all duration-100 ease-in-out print:hidden">
        <RotateCw strokeWidth={1.5} />
      </button>
      <div class="flex flex-col gap-1">
        <p>
          Created :
          <input bind:value={appState.invoice.created} type="text" size="10" placeholder="Date Created" maxlength="13" class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
        </p>
        <p>
          Due :
          <input bind:value={appState.invoice.due} type="text" size="10" placeholder="Due Date" maxlength="13" class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
        </p>
      </div>
  ...
  <div class="flex flex-row justify-between">
    <div class="flex flex-col gap-2">
      ...
      <textarea style="resize: none; padding: 8px 12px;" placeholder="Enter sender's name and address" cols="30" rows="3" maxlength="150" bind:value={appState.invoice.from} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]"></textarea>
      <div class="grid grid-cols-2 gap-2">
        <p class="flex items-center">
          <span class="w-24">Tax ID:</span>
          <input type="text" bind:value={appState.invoice.fromTaxId} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Tax ID" />
        </p>
        <p class="flex items-center">
          <span class="w-24">Email:</span>
          <input type="email" bind:value={appState.invoice.fromContact.mail} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Email" />
        </p>
        <p class="flex items-center">
          <span class="w-24">Phone:</span>
          <input type="text" bind:value={appState.invoice.fromContact.phone} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Phone" />
        </p>
      </div>
    </div>
    <div class="flex flex-col gap-2">
      <h4 class="mb-2 font-bold">To</h4>
      <textarea style="resize: none; padding: 8px 12px;" placeholder="Enter recipient's name and address" cols="30" rows="3" maxlength="150" bind:value={appState.invoice.to} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]"></textarea>
      <div class="grid grid-cols-2 gap-2">
        <p class="flex items-center">
          <span class="w-24">Tax ID:</span>
          <input type="text" bind:value={appState.invoice.toTaxId} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Tax ID" />
        </p>
        <p class="flex items-center">
          <span class="w-24">Email:</span>
          <input type="email" bind:value={appState.invoice.toContact.mail} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Email" />
        </p>
        <p class="flex items-center">
          <span class="w-24">Phone:</span>
          <input type="text" bind:value={appState.invoice.toContact.phone} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A] w-full" placeholder="Enter Phone" />
        </p>
      </div>
    </div>
  </div>
  ...
  <form class="flex flex-row justify-between gap-2">
    <button disabled={itemDesc == ''} on:click={() => addItem()} class="p-2 rounded-full bg-[#E2E2E2] hover:bg-[#CFCFCF] transition-all duration-100 ease-in-out print:hidden">
      <Plus stroke="#557571" />
    </button>

    <input id="descBox" type="text" bind:value={itemDesc} placeholder="Item name" class="border border-[#E2E2E2] rounded-lg p-3 focus:outline-none focus:border-[#5A5A5A] grow" on:keypress={(e) => e.key == 'Enter' && addItem()} />
    <input type="number" bind:value={itemPrice} placeholder="Unit Price" min="0" step="0.01" class="border border-[#E2E2E2] rounded-lg p-3 focus:outline-none focus:border-[#5A5A5A] w-32" on:keypress={(e) => e.key == 'Enter' && addItem()} />
    <input type="number" bind:value={itemQty} placeholder="Quantity" min="1" step="1" class="border border-[#E2E2E2] rounded-lg p-3 focus:outline-none focus:border-[#5A5A5A] w-32" on:keypress={(e) => e.key == 'Enter' && addItem()} />
  ...
  <div class="flex flex-col mt-4">
    ...
    <div class="flex flex-row even:bg-[#F8F8F8] border-t border-[#E2E2E2] items-center">
      ...
      <input class="p-3 w-32 text-right" type="text" bind:value={appState.discountPercent} disabled={!isDiscountEnabled} step="0.01" max="100" min="0" />
    </div>
    <div class="flex flex-row even:bg-[#F8F8F8] border-t border-[#E2E2E2] items-center">
  ...
  <div class="mt-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="flex flex-col">
        ...
        <input type="text" maxlength="20" bind:value={appState.payment.accountNumber} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </div>
      <div class="flex flex-col">
        <label class="text-sm font-semibold">Account Name</label>
        <input type="text" maxlength="28" bind:value={appState.payment.accountName} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </div>
      <div class="flex flex-col">
        <label class="text-sm font-semibold">Bank Name</label>
        <input type="text" maxlength="28" bind:value={appState.payment.bank} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </div>
      <div class="flex flex-col">
        <label class="text-sm font-semibold">IFSC</label>
        <input type="text" maxlength="11" bind:value={appState.payment.ifsc} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </div>
      <div class="flex flex-col">
        <label class="text-sm font-semibold">SWIFT Code</label>
        <input type="text" maxlength="11" bind:value={appState.payment.swiftCode} class="border border-[#E2E2E2] p-2 rounded-lg focus:outline-none focus:border-[#5A5A5A]" />
      </div>
    </div>
  </div>
  ...
  <div class="mt-6 flex justify-center print:hidden">
    <button on:click={() => window.print()} class="px-4 py-3 rounded-lg bg-[#E2E2E2] text-[#333] font-semibold flex items-center gap-2 hover:bg-[#CFCFCF] transition-transform duration-150">
